...
export const bannerConfigYupSchema = yup.object({
  ...
  startTime: yup
    .string()
    .optional()
    .test('is-date', 'Invalid startTime', (v) => !v || !isNaN(Date.parse(v))),
  endTime: yup
    .string()
    .optional()
    .test('is-date', 'Invalid endTime', (v) => !v || !isNaN(Date.parse(v))),
});
export const autoAlertConfigYupSchema = yup.object({
